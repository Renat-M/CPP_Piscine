...
/*   Updated: 2022/05/19 00:55:11 by meourea          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	{
        _hitPoints += amount;
        std::cout << _name << " is repaired with " << amount <<
        " points and now has " << _hitPoints << " hit points!" << std::endl;
        _energyPoints--;
        return ;
    }
    if (_hitPoints == 0)
        std::cout << _name << " has no hit points for repair!" << std::endl;
    else
        std::cout << _name << " has no energy points for repair!" << std::endl;
}
